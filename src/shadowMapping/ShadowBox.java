...
// area that is rendered from the perspective of the light source
public class ShadowBox {
	private static final float OFFSET = 10;
	private static final Vector4f UP = new Vector4f(0, 1, 0, 0);
	private static final Vector4f FORWARD = new Vector4f(0, 0, -1, 0);
	
	private float shadowDistance; // size of the view box, where shadows are mapped
	...
	public ShadowBox(Matrix4f lightViewMatrix, Camera camera, float shadowDistance) {
	...
	// recalculate shadow box with new camera orientation and position
	public void update() {
		...
		for (Vector4f point : points) {
			if (first) { // so we dont have to initialize negative and positive infinities
				minX = point.x;
				maxX = point.x;
				minY = point.y;
				maxY = point.y;
	...
	private void calculateWidthsAndHeights() {
		farWidth = (float) (shadowDistance * Math.tan(Math.toRadians(MasterRenderer.DEFAULT_FOV)));
		nearWidth = (float) (MasterRenderer.NEAR_PLANE * Math.tan(Math.toRadians(MasterRenderer.DEFAULT_FOV)));
